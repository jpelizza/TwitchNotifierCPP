...
void connectionManager::updateClientID(){
    ...
    if(curl) {
        ...
        headers = curl_slist_append(headers, ("Authorization: OAuth " + this->Oauth).c_str() );
        ...
        this->clientID= root.get("client_id",root).asString();
        this->userID = root.get("user_id",root).asString();
    }
}
...
void connectionManager::updateFollowers(){
    ...
    curl = curl_easy_init();
    if(curl) {
        std::string uri = "https://api.twitch.tv/helix/users/follows?from_id=" + this->userID;
...
void connectionManager::updateToNotify(){
    for(auto it = this->liveChannel.begin();
        it != this->liveChannel.end();
        it++){
            if(!(*it).notified){
                notify(*it);
                (*it).notified=true;
...
void connectionManager::notify(struct liveChannelInfo chan){

	auto Application = Gio::Application::create("stream.notification", Gio::APPLICATION_FLAGS_NONE);
	Application->register_application();
	auto Notification = Gio::Notification::create(chan.userName.c_str());
	Notification->set_body(chan.title.c_str());
    std::ifstream imageExists((this->imgPath + chan.userId + ".png").c_str());
    if(imageExists.fail()){
        downloadFollowsProfileImages();
    }
	auto Icon = Gio::ThemedIcon::create((this->imgPath + chan.userId + ".png").c_str());
	Notification->set_icon (Icon);
	Application->send_notification(Notification);

    std::cout << (this->imgPath + chan.userId + ".png") << std::endl;
}
